...
function makeSevenTypeSnapshot() {
  const evidence = [
    ...
    makeEvidence("ev_tiv", 2, "user", "TIV"),
    ...
    makeEvidence("ev_relation", 9, "user", "TIV는 Gemini를 사용한다")
  ];
  const topic = makeTopic("topic_launch", "ev_topic", {
    startMessageIndex: 1,
    endMessageIndex: 9
  });
  const tiv = makeEntity("entity_tiv", "ev_tiv", {
  ...
  return makeSnapshot(
    [
      ...
      makeProblem("problem_data", "ev_problem", {
        topicIds: ["topic_launch"],
        affectedEntityIds: ["entity_tiv"]
      }),
      makeChange("change_scope", "ev_change", {
        topicIds: ["topic_launch"],
        subjectEntityIds: ["entity_tiv"]
      }),
      gemini,
      makeRelation(
        "relation_tiv_gemini",
