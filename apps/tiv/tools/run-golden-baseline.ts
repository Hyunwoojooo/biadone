...
const contextMaxChars = positiveInteger(
  value("--context-max-chars", process.env.TIV_BASELINE_CONTEXT_MAX_CHARS ?? "16000")
);
...
async function loadConversation(
  ...
  const htmlPath = join(directory, `tiv-${compactId}.html`);
  try {
  ...
  const sourceHashes = await Promise.all(
    [...inputValue.sessions]
      ...
      .map(async (session) => {
        ...
        const html = await readFile(join(directory, `tiv-${compactId}.html`));
        return {
          sessionId: session.sessionId,
