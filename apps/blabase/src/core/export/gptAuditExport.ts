...
export function buildGptAuditMarkdown(input: GptAuditExportInput): string {
  ...
  return [
    "# blabase GPT Audit File",
    "",
    "이 파일은 ChatGPT에게 현재 blabase 분석 결과가 잘 분리/구조화됐는지 검수시키기 위한 자료입니다.",
    "아래 내용을 보고 Clean Conversation, Context Signals, Excluded/Internal 분리가 적절한지와 Sprint 3/4/5 구조화 및 근거 검증 결과가 원문과 맞는지 평가해주세요.",
    "",
    "## 1. Audit Questions",
    "",
    ...
    "9. Sprint 5B가 assistant-only 사용자 판단, 잘못된 quote/span, 불완전한 satisfaction pair를 적절히 Review 또는 Reject했나요?",
    "",
    "## 2. Analysis Metadata",
    "",
    fencedJson({
    ...
    "",
    "## 8. Sprint 5B Evidence Verification",
    "",
    hybridExtraction
      ? fencedJson({
          evidenceVerifier: hybridExtraction.evidenceVerifier,
          diagnostics: hybridExtraction.evidenceDiagnostics,
          verifiedItems: hybridExtraction.verifiedItems,
          reviewQueue: hybridExtraction.reviewQueue,
          rejectedItems: hybridExtraction.rejectedItems
        })
      : "_Evidence verification was not stored for this analysis._",
    "",
    "## 9. Clean Conversation Messages",
    "",
  ...
  return messages
    .map((message) =>
      [
        ...
        `createdAt: ${message.createdAt ?? "unavailable"}`,
        `updatedAt: ${message.updatedAt ?? "unavailable"}`,
