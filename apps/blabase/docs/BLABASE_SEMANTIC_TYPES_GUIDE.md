# blabase Semantic Types Guide

## 1. 문서 목적

이 문서는 blabase가 ChatGPT 공유 대화에서 추출하는 `SemanticItem`의 타입과 판정 기준을 설명한다.
...
따라서 blabase는 메시지 하나를 하나의 분류로 제한하지 않는다. 서로 다른 의미가 명시되어 있다면 하나의 메시지에서 여러 SemanticItem이 생성될 수 있다.
...
> blabase는 ChatGPTShareAdapter로 공유 HTML을 가져오고 Gemini를 Shadow Extractor로 사용한다.
...
> blabase는 Gemini API를 LLM Shadow Extractor로 사용한다.

```text
Source Entity: blabase
Relation: USES
