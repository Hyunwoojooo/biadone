...
이 문서는 blabase의 1차 사용자 제품이 대화를 정리할 때 사용하는 내부 의미 모델을 정의한다.

blabase Semantic Core v1은 다음 7개 타입만 사용한다.
...
blabase의 1차 제품은 사용자의 숨은 의도를 추론하거나 다음 행동을 제안하지 않는다.

핵심 약속은 다음과 같다.

> **blabase는 대화 안에서 실제로 표현된 목적, 주제, 조건, 문제, 변화, 대상, 관계를 원문 근거와 함께 정리한다.**
...
blabase에서 `객관적`이라는 말은 외부 세계에서 사실임을 보증한다는 뜻이 아니다.

blabase가 보증하는 것은 다음이다.
...
blabase가 기록할 수 있는 것은 다음과 같다.

```text
사용자가 시장 규모를 10조 원이라고 언급했다.
```

blabase가 검증하지 않은 채 기록해서는 안 되는 것은 다음이다.
...
```text
blabase는 Gemini를 사용한다.
blabase는 Gemini를 사용하지 않는다.
blabase가 Gemini를 사용한다면 비용을 확인해야 한다.
```
...
> blabase는 ChatGPT 공유 링크를 입력으로 사용하고 Gemini로 의미를 추출한다.
...
> blabase는 Gemini API를 의미 추출기로 사용한다.

```text
Relation
- blabase USES Gemini API
```
...
조건문만으로 현재 `blabase USES Gemini`를 확정할 수 없다.
...
> 처음에는 blabase v0.1을 대화 정리와 행동 제안을 함께 제공하는 형태로 출시하려 했지만, 의도 검증에는 사용자 데이터가 필요한데 아직 데이터가 부족해 검증이 어려워서 이제는 ChatGPT 대화 정리만 지원하는 버전으로 출시하고 행동 제안은 제외할 거야.

### 10.1 Intent

```text
blabase v0.1을 ChatGPT 대화 정리 중심으로 출시
```

### 10.2 Topic

```text
blabase v0.1 출시 범위 조정
```
...
```text
blabase v0.1 SUPPORTS ChatGPT 대화 정리
blabase v0.1 EXCLUDES 행동 제안
...
type PublicSummaryV1 = {
  schemaVersion: "blabase-public-summary.v1";
  analysisId: string;
...
| `RL-01` | “blabase는 Gemini API를 사용한다.”                                     | `blabase USES Gemini API`                                   |
| `RL-02` | “blabase와 Gemini API를 검토했다.”                                     | `USES` Relation 미생성                                  |
...
| `RL-06` | “blabase가 Gemini를 사용한다면”                                        | 조건부 관계이므로 Relation 미생성                       |
| `RL-07` | “blabase는 Gemini를 사용하지 않는다.”                                  | `USES`, `polarity = negated`                            |
...
blabase Semantic Core v1은 이를 즉시 삭제하거나 교체하지 않는다.
...
blabase Semantic Core v1은 다음 조건을 만족하면 구현 가능한 계약으로 간주한다.
