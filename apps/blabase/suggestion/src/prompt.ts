...
export function buildTaskCandidatePrompt(
  ...
  return {
    prompt: [
      ...
      "- Put only actionable non-terminal tasks in candidates. Allowed states there are open, in_progress, blocked, waiting, and unclear.",
      "- Put every identifiable completed, cancelled, or replaced update in the separate stateSignals array, even when no open task remains. Each state signal must include at least one exact state evidence quote.",
      "- Each stateSignals item contains only title, target, deliverable, state, and evidence. Do not include owner, origin, deadlineKind, deadlineText, or consequence in a state signal.",
      "- Return exactly one JSON object with both candidates and stateSignals. Return an empty array for either key when it has no items; never omit either key.",
      "- A planning option, undecided alternative, passive view, or possible future direction is not a task without an explicit user commitment, request, acceptance, blocker, or decision that the user must make now.",
      ...
      "- target is the concrete object or project. deliverable is the result the task must produce. Use stable concrete terms and preserve distinguishing scope such as production, staging, sandbox, recurring, or one-off. Do not add facts absent from evidence.",
      "- Every candidate needs exact contiguous quotes copied from allowed messages and an evidence kind.",
      "- deadlineKind is none unless an exact deadline phrase appears in deadline evidence. Copy the exact contiguous source substring, including the full ISO timestamp when present, into deadlineText without translating or reformatting it; otherwise use an empty string.",
