...
이 문서는 최초 M0 한 사이클 실험 당시의 역사적 스냅샷이다. 같은 턴 경계
1→2 반복 결과와 현재 책임 경계는 `T007B_C_HOOK_REPEAT_REPORT.md`가
대체한다.
...
이 보고서의 최초 검증 당시에는 같은 Codex 턴 안에서 두 번째 `emit_decision`을 호출하면 현재 턴 키와 충돌했다. 따라서 당시 M0은 반복 Pet 루프를 입증하지 않았고, 결정 경계 식별자를 확장하는 M1 상태 머신 설계가 필요했다. 이 제한은 후속 T-007b-C 스파이크에서 경계 두 개 범위로 갱신됐다.
