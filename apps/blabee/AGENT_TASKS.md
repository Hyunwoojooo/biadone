...
- T-012a는 읽기 전용 `blabee-coordinator doctor`와 순수 UDS `doctor_status` 기반을 구현했다. 앱·Codex exact 버전·Plugin 설치/source와 v0.1.0 정적 계약·PATH MCP identity·Hook 수동 검토·daemon 계약 호환·프로젝트 범위를 fail-closed로 진단하며 Doctor 18/18이 통과했다. allowlist는 비어 있고 실제 `.app`/서명·공증·DMG·signed Keychain·터미널 매트릭스와 전체 path TOCTOU/process-group hardening이 남아 있어 T-012는 `in_progress`다.
...
- T-012b-2는 단일 bundled coordinator의 제품 `service` 부트스트랩과 등록 전 LaunchAgent 계약을 구현했다. `service`는 exact app/executable/Resources/real Contracts와 Application Support 고정 경로만 받아 기존 daemon core로 연결하고, 추가 인자·환경 기반 경로·Keychain 시험 namespace를 거부한다. strict `service.json`은 current-user `0700` directory와 `0600` regular file을 descriptor 기반으로 bounded read한다. LaunchAgent는 exact 네 키와 `RunAtLoad`만 가지며 manifest와 app seal에 포함된다. Product 10/10, Swift Pet 65/65, 패키징 7/7, T-011 23/23, 계약 114/114, actual ad-hoc strict 서명과 plist 변조 거부가 통과했다. 실제 service/Keychain/SMAppService/launchctl은 실행하지 않았으므로 T-012는 `in_progress`다.
- T-010은 SwiftUI 기반 비활성 floating `NSPanel`, 다중 세션 카드와 명시적 foreground, 반고정 1·2·3·4 표시, 만료·위험 확인, 안전 범위의 단축키 설정 UI·동적 label과 PermissionRequest 알림을 구현했다. 활성 단축키 변경의 일부 등록 실패도 전체 후보를 폐기하고 이전 설정/binding으로 복원한다. 실제 WindowServer 1차 qualification에서 호스트 active/key/focus 유지, 비활성 Picker, 취소·격리 저장/재시작, 실제 Carbon 충돌, frame exact 왕복, 14-field focus와 16-field select를 통과했다. 발견한 접기/펼치기 위치 점프는 lower-trailing anchor 보존으로 수정했다. 실제 연속 시계에서는 reminder `60,056.709 ms`, expiry `120,030.318 ms`, 늦은 focus/select의 `interaction_not_waiting`과 최종 빈 routing 상태를 확인했다. Pet 35/35, Operational 14/14, Routing 필터 18/18(Routing 16 + Pet 2), Swift package XCTest 5/5 + Swift Testing 106/106가 통과했다. 다중 디스플레이 이동·분리, Spaces/전체 화면, 물리 전역 키·키보드 레이아웃, Pet 시각 만료·sleep과 실제 호스트 앱 복귀가 남아 있어 `in_progress`로 유지한다. 권한 요청의 원래 PID/창 identity가 없어 polling 시점 frontmost 앱으로만 best-effort 복귀하는 한계와 앱 번들/서명/DMG는 후속 범위다. 현재 operational 제안은 risk `info`, 빈 evidence, unavailable checkpoint, rollback disabled만 생성하므로 고위험 확인·채워진 상세·활성 롤백은 fixture 안전 경로만 검증됐다.
