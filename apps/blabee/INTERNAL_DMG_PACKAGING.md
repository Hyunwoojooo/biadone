...
현재 소스는 Codex `0.151.0`, `0.152.0`, `0.152.1`, `0.153.2`, `0.153.4`, `0.154.0`을 Plugin 연결
...
`0.154.0` Apple Silicon은 공식 패키지의 서명·공증, 격리된 Plugin 설치·조회·제거와
Hook 단위 계약 검사를 통과했다. 표준 Homebrew 실행 경로(`/opt/homebrew/bin/codex`)를
통한 Blabee의 실제 qualification·버전 조회·실행 직전 재검증도 통과했다.
이 지원은 새로 빌드한 앱부터 적용되며 기존 설치본에는
자동 반영되지 않는다. 실제 Hook→Pet→후속 실행 왕복은 별도 검증 대상이다.
build 35부터 `이번만 허용`은 공식 `0.153.4`·`0.154.0` arm64의 고정 서명 프로필과
실행 중인 Hook 호출 관계가 검증된 요청에서 활성화된다. 실제 설치 앱의 승인·명령 실행
왕복은 별도 검증 대상이며, 자격이 없는 요청은 `거절` 또는 `Codex에서 직접 선택`으로
처리한다. 관리형 App Server 자격은 확대하지 않는다.
