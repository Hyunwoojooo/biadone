...
| T-012d DMG 설치 후 Codex 간편 연결 | 소스·앱 패키징 계약 완료, 깨끗한 Mac 실사용 미검증 | 앱 Resources에 exact `blabee-app` local marketplace와 기존 Plugin payload를 함께 넣고, Pet 설정에서 사용자가 명시적으로 `Codex 연결하기`·업데이트·연결 해제를 실행할 수 있게 했다. 앱·설정 화면 열기는 Codex subprocess도 실행하지 않고, 명시적 동작 뒤 경로 identity·OpenAI 서명·Plugin 전용 지원 버전을 통과한 후보만 매 호출 직전 재검증한다. mutation은 single-flight이며 충돌·부분 설치·사후 상태 불일치를 성공으로 숨기지 않는다. 설치 성공 뒤에는 `Plugin 설치됨 · Hook 상태 확인`으로 표시하며 Blabee가 신뢰 완료를 자동 판정하지 못하므로 새 Codex의 `/hooks`에서 네 Hook을 사용자가 직접 확인하게 한다. 일반 `codex`, 공식 설치 파일, `.zshrc`, `PATH`, wrapper와 native `/resume`은 변경하지 않는다. 2026-09-03 안정화에서 Plugin manifest/ACL/lock/충돌 처리, 패키징 descriptor snapshot·조상 symlink 차단·서명 후 최종 entry 재검증·publish/cleanup, Pet 새로고침 경쟁, 테스트 자원 수명주기를 보강했다. 전체 Swift Testing 528/528+XCTest 5/5를 정상 권한에서 최종 12회 연속 통과했고 Plugin 집중 51/51, 전체 Node 304/304, 앱 조립 집중 27/27과 release build가 통과했다. 검증과 path 기반 실행·외부 Plugin 제거, 입력 검사와 복사, output publish 및 quarantine 정리 사이 same-user TOCTOU, NVM·Volta shim 호환성 및 다른 Mac의 DMG→연결→`/hooks`→Pet 왕복은 수동·후속 gate다. |
