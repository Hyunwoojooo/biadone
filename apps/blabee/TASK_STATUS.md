...
| 범위 | 결과 | 증거 |
|---|---|---|
| 전체 자동 테스트 | 조건부 통과 | 2026-09-05 최종 소스에서 전체 Node 346/346, Swift Testing 568/568+XCTest 5/5 통과. 과거 단계별 결과와 일시적 환경 실패는 아래 이력에 별도로 보존한다. |
...
| T-012b-2 제품 service·정적 LaunchAgent | foreground 자격과 r8 로컬 등록·실행 확인, 공개 자동 시작 미승인 | `service`는 exact app/Resources/real Contracts와 Application Support 고정 경로만 사용하고 추가 인자·환경 경로 우회를 거부한다. strict `service.json`과 정적 LaunchAgent exact 네 키를 구현했다. r8 설치 직후 기존 ad-hoc LWCR 갱신은 실패했지만 앱의 명시적 서비스 재시작이 unregister/re-register와 새 BTM 등록을 수행한 뒤 launchctl running·service 응답을 확인했다. Developer ID·공증 빌드의 clean install/update와 로그인 재현은 미승인이다. |
...
| T-012b-3b Pet 온보딩 UI·서비스 상태 계약 | UI·자동 회귀·r8 로컬 service 복구 확인, 공개 자동 시작 미승인 | 수동 상태/설정 조회, 명시적 버튼 전용 등록·해제·System Settings·프로젝트 변경, single-flight와 configured/active 분리를 구현했다. coordinator transport와 `SMAppService` 등록 상태를 분리해 transport 오류를 등록 해제로 오인하거나 자동 재등록하지 않으며, 등록됐지만 응답이 없으면 `실행 확인 필요`로 표시한다. r8 첫 실행의 LWCR 갱신 실패 뒤 명시적 서비스 재시작이 unregister/re-register와 새 BTM 등록을 수행했고 launchctl running·service 응답을 확인했다. 최신 전체 Swift Testing 568/568+XCTest 5/5가 통과했다. Developer ID 기반 seamless upgrade는 미승인이다. |
| T-012c 내부 테스트용 DMG | r8 자동 패키징·한 개발 Mac 설치 확인, clean Mac·공개 배포 미승인 | 현재 소스의 Swift·Contracts·Plugin·Packaging을 하나의 private snapshot으로 고정해 release build하고, ad-hoc 서명한 `Blabee.app`, `Applications` 링크, `INTERNAL_TESTING.txt`만 담은 DMG와 SHA-256 sidecar를 생성한다. 구형 prebuilt `--binary` 직접 패키징은 차단했다. transaction schema v2는 build number와 expected architecture `arm64`를 봉인하고 legacy v1 복구를 fail-closed한다. 조립·mount·최종 결과는 exact arm64만 허용한다. r8 앱 `0.1.0` build `8`의 DMG SHA-256은 `e0dbe4ff31713a76df9b38dd3e94f799d53f1bbfec86350cc28d35afd2bffa31`이며 전체 Node 346/346이 통과했다. 설치본 metadata·arm64·deep/strict ad-hoc 서명도 확인했다. Developer ID·DMG 서명·공증·stapling·자동 업데이트는 포함하지 않고 `public_distribution_ready = false`다. |
| T-012d DMG 설치 후 Codex 간편 연결 | 소스·앱 패키징 계약과 r8 로컬 Plugin 상태 표시 확인, 깨끗한 Mac 왕복 미검증 | 앱 Resources에 exact `blabee-app` local marketplace와 기존 Plugin payload를 함께 넣고, Pet 설정에서 사용자가 명시적으로 `Codex 연결하기`·업데이트·연결 해제를 실행할 수 있게 했다. 앱·설정 화면 열기는 Codex subprocess도 실행하지 않고, 명시적 동작 뒤 경로 identity·OpenAI 서명·Plugin 전용 지원 버전을 통과한 후보만 매 호출 직전 재검증한다. mutation은 single-flight이며 충돌·부분 설치·사후 상태 불일치를 성공으로 숨기지 않는다. r8 설정은 `Plugin 설치됨 · Hook 상태 확인`을 표시했고 transport 오류는 없었다. 일반 `codex`, 공식 설치 파일, `.zshrc`, `PATH`, wrapper와 native `/resume`은 변경하지 않는다. Codex `0.153.2` Plugin CLI 호환성은 지원하지만 managed runtime identity/version/code-mode allowlist는 별개로 실패하며 관리형 승인 지원을 주장하지 않는다. 검증과 path 기반 실행·외부 Plugin 제거, 입력 검사와 복사, output publish 및 quarantine 정리 사이 same-user TOCTOU, NVM·Volta shim 호환성 및 다른 Mac의 DMG→연결→`/hooks`→Pet 왕복은 수동·후속 gate다. |
| Codex `0.153.2` Plugin CLI 호환성 (2026-09-04) | 로컬 계약·자동 회귀·r8 설치본 Plugin 상태 표시 확인, clean Mac 왕복 미검증 | 공식 changelog상 `0.153.2`는 Fast tier 설명만 바뀌었고 remote marketplace는 `0.153.0`에서 도입됐다. 로컬 command/help/list 계약과 격리 Plugin lifecycle 2/2를 확인했다. remote installed record의 `source.path` 누락은 non-local record에만 허용하며 Blabee Plugin은 exact local path를 계속 강제한다. 공식 arm64 archive SHA-256은 `287e2dd0a9bbfb58581b0a9150399458b4f094ea42caf02860f1e8cb5a202a0b`, executable SHA-256은 `195ace4100a634a9df39147f493e730e666b5bd87795f3c9f3251d8542400424`다. 공식 bytes의 strict Apple signature 실패 때문에 arm64 `0.153.2` exact-binary hash에만 좁은 예외를 두고 구조·identity·owner·mode·ACL과 다른 binary의 정상 서명 경로는 유지한다. 선택·고정된 실행 파일의 전체 hash는 connect·disconnect·구형 migration 작업당 한 번만 재사용하고 탐색 중 거부된 다른 후보와 구분한다. 최종 전체 Node 337/337, `npm run test:swift` Swift Testing 555/555+XCTest 5/5, Plugin·호환성 집중 Swift 78/78, AppPackaging 73/73과 r4 DMG 이미지·서명 검증이 통과했다. r8은 `/Applications`에 설치됐고 Pet 설정에 Plugin installed/Hook guidance가 표시됐다. 이는 Plugin CLI 자격일 뿐 managed App Server/runtime 승인이 아니며, 설치된 Doctor의 managed identity/version/code-mode allowlist 실패를 우회하지 않는다. clean Mac Hook/Pet 왕복·connect hash I/O 측정·기존 URL-spawn TOCTOU는 pending이다. |
...
- 진행 중: T-004, T-010, T-011, T-012. T-015의 실제 `답변 완료 → FIFO Pet 선택 → 같은 세션 새 턴` 왕복은 완료했다. T-010은 확장 카드 시각 캡처·디스플레이·Space·키보드 레이아웃·sleep·호스트 앱 매트릭스, T-011은 실제 sleep/복귀, T-012는 내부 DMG의 깨끗한 Mac 스모크와 그 환경의 서비스 등록/승인·signed Keychain·Developer ID·공증·공개 DMG·버전 allowlist·터미널 매트릭스가 남았다.
...
6. r8의 한 개발 Mac service 복구 확인에 이어 깨끗한 Mac 스모크, signed Data Protection Keychain, Developer ID·공증·공개 DMG·updater, exact Codex allowlist와 Terminal/iTerm/VS Code/Orca 매트릭스를 순서대로 검증한다.
...
- T-012b-3b는 `.enabled`를 등록·실행 자격 상태로만 표시하고 daemon health로 해석하지 않는다. 앱 시작·poll·snapshot·설정 화면 열기는 `SMAppService.register()`를 호출하지 않는다. r8에서 명시적 서비스 재시작의 unregister/re-register와 daemon 응답은 확인했지만, ad-hoc LWCR 복구 경로이므로 Developer ID 서명 앱의 clean install/update 자격은 아니다. 설정 변경은 service를 자동 재시작하지 않아 configured와 active가 다음 재시작까지 다를 수 있다.
