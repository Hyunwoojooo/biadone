...
| T-012b-3a 프로젝트 설정 writer | 안전한 설정 변경 계약 자격 완료, 제품 UI·자동 시작 미승인 | exact 앱 전용 `project-settings`, current-user 0700/0600·single-link 경계, mutex+flock, strict locked RMW, file/directory fsync와 atomic rename을 구현했다. reader도 Application Support ancestor symlink를 거부한다. Writer 12/12, fresh reader+writer 23/23, Swift Pet 78/78, 전체 Swift Testing 150/150+XCTest 5/5, release build와 패키징 7/7 통과. 실제 service, Keychain, SMAppService, launchctl, 설치는 실행하지 않음 |
| T-012b-3b Pet 온보딩 UI·서비스 상태 계약 | UI·fake adapter 자격 완료, 실제 자동 시작 미승인 | 수동 상태/설정 조회, 명시적 버튼 전용 등록·해제·System Settings·프로젝트 변경, single-flight와 변경 후 reread, configured/active 분리를 구현했다. Onboarding 10/10, Pet 88/88, 전체 Swift Testing 160/160+XCTest 5/5, T-011 23/23, 패키징 7/7, 계약 114/114, release/ad-hoc strict 검증 통과. 실제 SMAppService, service, NSOpenPanel, System Settings, 사용자 Application Support와 제품 primary Keychain은 변경하지 않음. 전체 Swift 회귀의 격리된 test-only Keychain account는 정리함 |
...
4. T-012b-3c 실기기 gate에서 signed `Blabee.app`의 실제 `SMAppService` 등록·승인 필요 상태·System Settings 이동·해제와 로그인/로그아웃·재부팅 수명주기를 검증한다. 등록은 곧 service를 시작할 수 있으므로 사용자 동의를 받은 뒤에만 수행한다. 그 뒤 signed Data Protection Keychain/access group·`LAContext`, Developer ID 서명·공증·DMG·updater, exact Codex allowlist, `/hooks` 신뢰와 Terminal/iTerm/VS Code/Orca 매트릭스를 순서대로 검증한다. 현재 Pet UI와 fake adapter 계약까지만 자격을 통과했으며 실제 자동 시작·공개 배포 identity·공증은 검증하지 않았다.
