...
- 작성일: 2026-09-05
- 갱신일: 2026-09-08 (앱 실행형 서비스 · 내부 build 14)
...
중단하고 빌드 번호·시각·증상을 전달한다. 자세한 내용은 `KNOWN_ISSUES.md`의
`BLB-APP-SERVICE-001`, `BLB-APP-SERVICE-002`를 참고한다.
...
2026-09-08 패키지 무결성을 검증한 내부 테스트 후보는 다음 두 파일이다.

```text
Blabee-0.1.0-internal-arm64-20260908-r14.dmg
Blabee-0.1.0-internal-arm64-20260908-r14.dmg.sha256
```

두 파일에서 확인해야 할 SHA-256은
`1318370593222d207435c55544260d08bf808cfb43d9155b0d772215ab48db51`다.
앱 버전은 `0.1.0`, 내부 build number는 `14`, 아키텍처는 정확히 `arm64`다.
파일명이 다르거나 sidecar의 값이 다르면 사용하지 않는다. r14는 ad-hoc 서명 앱을 담은
미서명·미공증 DMG이며 `public_distribution_ready = false`다. r8 등 이전 DMG는
현재 후보로 대체된 로컬 산출물이므로 새 테스터에게 배포하지 않는다.

전달 시 이 `.md` 가이드도 함께 보낸다. 이전 r8 기준 PDF는 갱신하지 않았으므로
r14 설치 안내로 전달하지 않는다. r14의 패키징 검증과 기존 build 13의 실사용 검증은
[r14 릴리스 기록](INTERNAL_DMG_R14_RELEASE_KO.md)에서 구분한다.
