...
| ID | 최초 확인일 | 심각도 | 상태 | 요약 |
|---|---|---:|---|---|
| [`BLB-INSTALL-001`](INSTALLER_RUNTIME_LEASE_KO.md) | 2026-09-10 | 중간 | 검증 중 | 다른 앱의 경로 조회 실패로 종료 후에도 설치 차단; 검증된 릴리스 간 실행 잠금 구현, 구형 첫 이관·설치본 검증은 별도 |
| [`BLB-HOOK-APPROVAL-001`](PERMISSION_CARD_APPROVAL_KO.md) | 2026-09-09 | 중간 | 검증 중 | 서명 조회 플래그 누락으로 정상 Codex의 일회성 승인 자격이 탈락함; 조회 수정·세 버튼 고정 구현, 설치본 왕복 검증 대기 |
| [`BLB-CONNECTION-UX-001`](CONNECTION_STATUS_UI_KO.md) | 2026-09-09 | 중간 | 검증 중 | 서비스·Plugin·프로젝트 적용·실제 동작이 혼동됨; 연결 요약과 단계별 안내 구현, 배포본 반영·다른 Mac 검증 대기 |
| `BLB-SHELL-001` | 2026-09-08 | 중간 | 검증 중 | 구형 v4 래퍼와 공식 Codex가 다르게 동작함; 제한적 정리 구현 후 실제 PC 이관 미검증 |
| `BLB-RECHECK-001` | 2026-09-08 | 중간 | 검증 중 | 명시적 Codex 재검사 진행·동일 오류 재완료 표시 구현; 설치본 UI 확인 대기 |
...
## BLB-SHELL-001 / BLB-RECHECK-001 — 구형 셸 연결과 검사 피드백

- 확인·기록일: 2026-09-08
- 상태: 소스 구현 완료, 검증 중; 격리 자동 검증과 실제 테스터 PC 검증은 구분한다.
- 보고된 사실: 테스터 앱은 build 15, 디스크 v4 래퍼는 이미 `.local/bin/codex`로 수정됐다.
  원래 대화형 터미널의 로드된 함수와 앱의 상세 연결 오류는 미확인이다.
- 현재 구현은 공식 Codex를 직접 탐색한다. 구형 셸 함수가 앱 연결 실패의 원인이라고 단정하지 않는다.
- 수정 범위: exact-v4 전용 래퍼의 명시적 백업·비활성화, 카드 근처 재검사 진행/완료 시각/실패 단계,
  비식별화 진단 복사. `.zshrc`와 Codex 자체는 변경하지 않는다.
- 수동 수정된 래퍼는 자동 정리 대상이 아니다. 새 터미널 및 Hook·서비스·카드·선택 반환 확인 전에는
  원격 문제가 해결됐다고 표시하지 않는다. 외부 Computer Use `-10005`는 Blabee 진단 코드가 아니다.
- 설계·제한·검증 기록: [구형 연결 복구 계획](CODEX_LEGACY_RECOVERY_KO.md).
